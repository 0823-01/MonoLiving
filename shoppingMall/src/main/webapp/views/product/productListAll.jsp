			    ...
			    <ul class="product-grid">
			        <% for(ProductInfo p : list) { %>
			            <li class="product-item" data-pno="<%= p.getProductNo() %>">
			                <img src="/admin/<%= p.getTitleImg() %>" alt="<%= p.getProductName() %>">
			                <div class="product-info">
			                    <div class="product-details">
			                        <p><%= p.getProductName().length() <= 10 ? p.getProductName() : p.getProductName().substring(0, 9) + "..." %></p>
			                        <span><%= String.format("%,d", p.getPrice()) %>원</span> <!-- 가격에 콤마 추가 -->
			                    </div>
